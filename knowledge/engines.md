...
`distribute_into_installments` walks installments oldest-first. When an installment's per-component allocation does not cover its remaining balance, `_absorb` pulls from the remaining component pools (fine -> mora -> interest -> principal) up to the shortfall. After residual post-processing, `_finalize_coverage` projects a hypothetical post-payment `Installment` per allocation and reads `Installment.is_fully_paid` directly — by construction the allocation flag equals the installment flag for the targeted installment, regardless of payment timing, anticipation, waivers, or per-cycle mora rate. Flag *monotonicity* (no True after False) is not enforced: when an earlier installment runs out of mora pool but its principal is covered, a later installment can legitimately be fully covered. The real "no principal leak" invariant is enforced by the oldest-first principal flow itself and is asserted in `tests/invariants/test_sequential_coverage.py`.
...
- **`balance_tolerance` is a loan-level setting**: `Loan` and `BillingCycleLoan` accept a `balance_tolerance: Optional[Money]` constructor parameter (default `Money("0.01")` via `DEFAULT_BALANCE_TOLERANCE`). The value lives on `BaseLoan.balance_tolerance` and is threaded through every engine function that takes tolerance-based decisions (`compute_state`, `_build_installments_snapshot`, `build_installments`, `distribute_into_installments`, `allocate_payment_into_installments`, `_finalize_coverage`, `_reconcile_coverage_with_final_state`, `principal_covered_count`, `fully_covered_count`, `_prior_underpaid_interest`, `_accrual_end_with_waiver_cap`, `compute_fines_at` / `_has_payment_near`). Each function defaults to the engine-wide `BALANCE_TOLERANCE` constant so existing callers stay working; the loan attribute is what end users override. `Installment.from_schedule_entry` carries the value into the constructed installment via the `balance_tolerance` field, keeping the model and the engine in lockstep — there is no longer a duplicated constant in `models/installment.py`.
