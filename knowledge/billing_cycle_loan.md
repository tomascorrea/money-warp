...
- **Resolver is optional**: without it, behavior is identical to a `Loan` with billing-cycle-derived due dates. This invariant is formally verified by a Hypothesis property test (`tests/test_schedule_equivalence.py`) that generates random loan parameters and asserts both classes produce identical schedules.
