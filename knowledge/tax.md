...
- Each IOF installment amount is rounded to 2 decimal places (real money) via `.to_real_money()` before storing in the per-installment detail and accumulating in the total. This applies to both PRECISE and PER_COMPONENT modes. In PRECISE mode, the daily and additional components are summed first, then rounded once per installment. In PER_COMPONENT mode, each component is rounded individually, then summed and rounded again (the second round is a no-op when both inputs are already clean cents). Because rounding happens per installment, the total may differ by up to a few cents from a system that sums high-precision values and rounds once at the end.
- External validation against a Brazilian lending platform shows our PER_COMPONENT mode matches periods 1-11 exactly. Period 12 differs by 1 cent due to our last-payment-by-difference approach in PriceScheduler. For PRECISE mode, the per-installment rounding can cause the total to differ by up to 2 cents from the external system's total.
