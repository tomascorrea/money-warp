...
def _make_loan_with_all_installments_paid() -> tuple:
    """Helper: create a 3-payment loan and pay every scheduled installment exactly."""
    principal = Money("1000.00")
    rate = InterestRate("5% a")
...
def test_loan_current_balance_zero_after_all_installments_paid():
    """Paying every scheduled installment must result in a zero current balance."""
    loan, due_dates = _make_loan_with_all_installments_paid()

    with Warp(loan, due_dates[-1]) as warped_loan:
        assert warped_loan.current_balance == Money.zero()


def test_loan_principal_balance_zero_after_all_installments_paid():
...
def test_loan_accrued_interest_zero_after_all_installments_paid():
...
def test_loan_no_outstanding_fines_after_on_time_installments():
...
def _make_partial_payment_loan() -> tuple:
    """Helper: loan with a single partial payment recorded on the due date."""
    principal = Money("1000.00")
    rate = InterestRate("5% a")
...
def test_loan_principal_balance_reduced_at_payment_date_with_warp():
    """Warp to payment date must show a principal balance lower than original."""
    loan, principal, payment_date = _make_partial_payment_loan()

    with Warp(loan, payment_date) as warped_loan:
        assert warped_loan.principal_balance < principal


def test_loan_accrued_interest_non_negative_at_payment_date_with_warp():
    """Accrued interest at the payment date must be non-negative."""
    loan, _, payment_date = _make_partial_payment_loan()

    with Warp(loan, payment_date) as warped_loan:
        assert warped_loan.accrued_interest >= Money.zero()


def test_loan_current_balance_equals_components_at_payment_date_with_warp():
...
def test_loan_interest_accrues_after_partial_payment_with_warp():
